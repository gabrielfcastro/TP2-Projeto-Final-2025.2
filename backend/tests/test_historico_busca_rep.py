...
from app.models import historico_busca_rep
...
def test_adicionar_historico_busca_sucesso(setup_usuario):    
    ...
    id_historico = historico_busca_rep.adicionar_historico_busca(usuario_id=setup_usuario, produto_buscado="Maçã")
    assert id_historico is not None
    
    historico_db = historico_busca_rep.buscar_historico_por_id(id_historico)
    assert historico_db['produto_buscado'] == "Maçã"
    assert historico_db['usuario_id'] == setup_usuario
    assert historico_db['feirante_buscado'] == ""
    
    historico_busca_rep.deletar_historico_busca(id_historico)

def test_adicionar_historico_busca_feirante(setup_usuario):
    
    id_historico = historico_busca_rep.adicionar_historico_busca(usuario_id=setup_usuario, feirante_buscado="Feirante João")
    assert id_historico is not None
...
def test_adicionar_historico_busca_completo(setup_usuario):
    ...
    id_historico = historico_busca_rep.adicionar_historico_busca(usuario_id=setup_usuario, produto_buscado="Banana", feirante_buscado="Feirante Maria")
    assert id_historico is not None
...
def test_validacao_parametros_vazios(setup_usuario):
    ...
    with pytest.raises(ValueError) as error_info:
        historico_busca_rep.adicionar_historico_busca(usuario_id=setup_usuario)
    assert "pelo menos um parâmetro de busca" in str(error_info.value).lower()

def test_listar_historico_usuario(setup_usuario):
    
    id1 = historico_busca_rep.adicionar_historico_busca(setup_usuario, "Maçã")
    id2 = historico_busca_rep.adicionar_historico_busca(setup_usuario, "Banana")
    id3 = historico_busca_rep.adicionar_historico_busca(setup_usuario, feirante_buscado="Feirante Teste")
    
    lista = historico_busca_rep.listar_historico_buscas(usuario_id=setup_usuario)
    ...
    historico_busca_rep.deletar_historico_busca(id1)
    historico_busca_rep.deletar_historico_busca(id2)
    historico_busca_rep.deletar_historico_busca(id3)

def test_buscar_historico_por_id(setup_usuario):
    
    id_historico = historico_busca_rep.adicionar_historico_busca(setup_usuario, "Laranja", "Feirante Citrus")
    
    historico = historico_busca_rep.buscar_historico_por_id(id_historico)
    ...
    historico_inexistente = historico_busca_rep.buscar_historico_por_id(99999)
...
def test_deletar_historico_busca(setup_usuario):
    ...
    id_historico = historico_busca_rep.adicionar_historico_busca(setup_usuario, "Produto para Deletar")
    
    assert historico_busca_rep.buscar_historico_por_id(id_historico) is not None
    
    historico_busca_rep.deletar_historico_busca(id_historico)
    
    assert historico_busca_rep.buscar_historico_por_id(id_historico) is None