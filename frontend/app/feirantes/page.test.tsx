...
test("deve exibir informações completas do feirante", () => {
  ...
  expect(
    screen.getByText("Produtos frescos direto da roça"),
  ).toBeInTheDocument();
...
test("deve exibir formulário de cadastro na visão feirante", () => {
  ...
  fireEvent.click(screen.getByText("Feirante"));
  expect(
    screen.getByPlaceholderText("Ex: Tomate Italiano"),
  ).toBeInTheDocument();
  expect(
    screen.getByPlaceholderText("Ex: Produto fresco colhido hoje"),
  ).toBeInTheDocument();
...
test("não deve adicionar produto sem nome e preço", () => {
  ...
  fireEvent.click(screen.getByText("+ Adicionar Produto"));
  expect(screen.getAllByText("Excluir Produto")).toHaveLength(
    quantidadeInicial,
  );
});
