...
def test_criar_deletar_usuario():
    try:
        ...
        assert usuario_db['email'] == "paulo4@gmail.com"
        assert usuario_db['nome'] == "Paulo Alves"
        assert usuario_db['tipo_usuario'] == "Cliente"

        assert usuario_rep.verificar_credenciais(
            usuario_id, 
            senha_enviada="!Paulo123"
        ) is not None
        
    finally:      
        try:
            usuario_rep.deletar_usuario(usuario_id)
            print(f"Usuário de CPF {usuario_id} removido com sucesso.")
            
        except Exception as e:
            print(f"Falha ao limpar usuário de CPF {usuario_id}: {e}")
